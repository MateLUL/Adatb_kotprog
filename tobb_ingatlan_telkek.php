...
<body>
    ...
    $query = $csatlakozas->query("SELECT ttb.helyrajzi_szam, ttb.azonosito
    ...
    if ($query->num_rows > 0) {
        ...
        foreach ($query as $telek) {
            if ($elozo_helyrajzi_szam != $telek['helyrajzi_szam']) {
                echo "<a href=\"./telkek_listazasa_oldal.php?helyrajzi_szam=" . $telek['helyrajzi_szam'] . "&helyrajzi_szam_alapjan=Helyrajzi+szám+alapján+keresés\">Telek és tulajdonos(ok) megtekintése</a><br>";
                echo "Helyrajzi szám: " . $telek['helyrajzi_szam'] . "<br>" . "Tulajdonos: " . $telek['azonosito'] .
                "<br>";
            } else {
                echo "Tulajdonos: " . $telek['azonosito'] . "<br><br>";
            }
